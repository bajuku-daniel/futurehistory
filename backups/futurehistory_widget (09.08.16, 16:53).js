    ...
    attach: function(context, settings) {
      ...
      $.each(Drupal.settings.futurehistory.defaults, function(i, mapDefaults) {
        ...
        $("#futurehistory-map-" + i).once('futurehistory-googlemaps', function(){
          ...
          direction = $('#futurehistory-view_direction-slider-' + i + '-value input').attr('value') == false ? parseFloat(mapDefaults.direction) : $('#futurehistory-view_direction-slider-' + i + '-value input').attr('value');
          angle = $('#futurehistory-angle-slider-' + i + ' input').attr('value') == false ? parseFloat(mapDefaults.angle) : $('#futurehistory-angle-slider-' + i + ' input').attr('value');
          ...
          // PREV NEXT Button Funktion for the edit and add picture form
          ...
          $('a[href="#field_position_der_aufnahme"]').on('shown.bs.tab', function(e) {   // When tab is displayed...
            center = Drupal.futurehistory.maps[i].getCenter();
            google.maps.event.trigger(Drupal.futurehistory.maps[i], 'resize');         // fixes map display
            Drupal.futurehistory.maps[i].setCenter(center);                            // centers map correctly
          });
          ...
          $("#futurehistory-arview-" + i + " input").click(function(){ 
            if ($(this).is(':checked')){
              ...
              Drupal.futurehistory.angle_sliders[i].slider( "option", "values", [ 60, 100]);
          ...
          Drupal.futurehistory.angle_sliders[i] = $("#futurehistory-angle-slider-" + i + " ").slider({
            ...
            slide : function( event, ui ) {
              ...
              console.log('range min:' + range_min + 'mid' + mid + 'last range min' + last_range_min );
